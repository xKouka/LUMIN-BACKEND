...
/**
 * Envía un correo de notificación cuando cambia el estado de la muestra
 * @param {string} recipientEmail - Email del destinatario
 * @param {string} nombre - Nombre del paciente
 * @param {string} estado - Nuevo estado (en_proceso, completado)
...
const sendStatusUpdateEmail = async (recipientEmail, nombre, estado, idMuestra) => {
  try {
    ...
    const isConnected = await verifyConnection();
    if (!isConnected) {
      console.warn('⚠️ No hay conexión SMTP, omitiendo envío de email de estado');
      return { success: false, error: 'No SMTP connection' };
    }
    ...
    if (estado === 'en_proceso') {
      subject = `⏳ Tu muestra #${idMuestra} está siendo procesada - L.U.M.I.N.`;
      title = 'Tu muestra está en proceso';
      message = 'Te informamos que tus muestras han comenzado a ser analizadas por nuestro equipo de laboratorio. Te notificaremos nuevamente cuando los resultados estén listos.';
      color = '#3b82f6'; // Azul
    } else if (estado === 'completado') {
      subject = `✅ Resultados listos: Muestra #${idMuestra} - L.U.M.I.N.`;
      title = '¡Tus resultados están listos!';
      message = 'El análisis de tus muestras ha finalizado. Ya puedes acceder a tu cuenta para ver y descargar los resultados detallados.';
      color = '#10b981'; // Verde
    } else {
      return { success: false, error: 'Estado no requiere notificación' };
    }
    ...
    const mailOptions = {
      ...
      <head>
        ...
        <style>
          body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0; }
          .container { max-width: 600px; margin: 0 auto; padding: 20px; }
          .header { text-align: center; padding: 20px 0; background: linear-gradient(135deg, ${color} 0%, ${estado === 'en_proceso' ? '#1d4ed8' : '#059669'} 100%); border-radius: 10px 10px 0 0; color: white; }
          .content { background: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px; }
          .button { display: inline-block; padding: 12px 30px; background-color: ${color}; color: white; text-decoration: none; border-radius: 5px; margin: 20px 0; font-weight: bold; }
          .footer { text-align: center; margin-top: 20px; font-size: 12px; color: #666; }
        </style>
      </head>
      <body>
        <div class="container">
          ...
          <div class="content">
            ...
            <div style="text-align: center;">
              <a href="http://localhost:3000/login" class="button">Ver Resultados</a>
            </div>
    ...
    const info = await transporter.sendMail(mailOptions);
    console.log(`✓ Email de estado (${estado}) enviado:`, info.messageId);
    return { success: true, messageId: info.messageId };
  } catch (error) {
